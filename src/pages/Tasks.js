  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        fetchedTasks.sort((a,b) => new Date(a.created_at) - new Date(b.created_at));
...
//----------------------------------------------------------------------------------------------------------------------------------------//
  return (
    <div className="tasks-container">
      ...
      <Button onClick={() => setAddTaskDialogOpen(true)} className="add-task-button" variant="contained" style={{ marginBottom: '20px' , background: '#266775' }}>
        Add Task
      </Button>
      ...
      {taskList.map((task) => {
        ...
        <div key={task.id} className="task-row">
          ...
          <div className="tasks-column task-comments"><CommentsLabel comments={task.comment_count} /></div>
